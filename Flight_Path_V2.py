...
try:
    ...
    os.mkdir(os.path.join(path_in,dirName))
    print("Directory " , dirName ,  " Created ")
except FileExistsError:
    #check to see if the directory already exisits
    print("Directory " , dirName ,  " already exists")
    shutil.rmtree(os.path.join(path_in,dirName))
    print("Deleting old directory and creating a new one")
    os.mkdir(os.path.join(path_in,dirName))
    print("Directory " , dirName ,  " Created ")
    
    
# Get the path of our new folder
path_out = os.path.join(path_in,dirName)
# Get the files in our current folder
files = glob.glob(os.path.join(path_in, "*.txt"))

# Open and get information from all files
for File_Name in files:
    with open(File_Name, 'r') as f:
        lines = f.readlines()[23:]
        if '        Latitude        Longitude        H-Ell         Latitude        Longitude        H-Ell      CorrTime        Heading          Pitch           Roll     VEast    VNorth       VUp\n' in lines:
            new_read_point = lines.index('        Latitude        Longitude        H-Ell         Latitude        Longitude        H-Ell      CorrTime        Heading          Pitch           Roll     VEast    VNorth       VUp\n') + 2
    ...
    shutil.move(New_File_Name, path_out)

print("All files have been converted.")

print("Please copy the following file location into the browser:")

print(path_out)

os.startfile("Text_to_Shape_NAD83_2011.gms")
t = time.sleep(20)
print("This window will close in %d seconds" %t)
